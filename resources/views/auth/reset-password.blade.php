...
<head>
    ...
    <title>{{ FrontHelper::getAppName() }} - Réinitialisation de mot de passe</title>
                        ...
                        <div class="row">
                            <div class="col-md-10 offset-md-1">
                                ...
                                <div class="my-5 text-center text-lg-start">
                                    <h1 class="display-8">Réinitialisation en cours!!!</h1>
                                    @include('back.layouts.partials.alert')
                                </div>
                                <form class="mb-5" method="POST" action="{{ route('password.store') }}">
                                    @csrf
                                    <input type="hidden" name="token" value="{{ $request->route('token') }}">
                                    <div class="mb-3">
                                        <input type="email" name="email" class="form-control" value="{{ old('email', $request->email) }}" placeholder="Entrer votre email" autofocus required>
                                        @error('email')
