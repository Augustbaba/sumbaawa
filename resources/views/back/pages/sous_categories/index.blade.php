@extends('back.layouts.master')

@section('title', 'Liste des Sous-Catégories')

@section('content')
<div class="content">
    <div class="mb-4">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="{{ route('dashboard') }}">
                        <i class="bi bi-globe2 small me-2"></i> Dashboard
                    </a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">Sous-Catégories</li>
            </ol>
        </nav>
    </div>

    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="card-title"><i class="bi bi-list-ul me-2"></i>Liste des Sous-Catégories</h5>
                <a href="{{ route('sous-categories.create') }}" class="btn btn-primary">
                    <i class="bi bi-plus-circle me-2"></i>Nouvelle Sous-Catégorie
                </a>
            </div>

            @if (session('success'))
                <div class="alert alert-success">
                    {{ session('success') }}
                </div>
            @endif

            <div class="table-responsive">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Nom</th>
                            <th>Slug</th>
                            <th>Catégorie</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($sousCategories as $key => $sousCategorie)
                            <tr>
                                <td>{{ $sousCategories->firstItem() + $key }}</td>
                                <td>{{ $sousCategorie->label }}</td>
                                <td>{{ $sousCategorie->slug }}</td>
                                <td>{{ $sousCategorie->categorie ? $sousCategorie->categorie->label : 'N/A' }}</td>
                                <td>
                                    <a href="{{ route('sous-categories.show', $sousCategorie) }}"
                                       class="btn btn-sm btn-info">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <a href="{{ route('sous-categories.edit', $sousCategorie) }}"
                                       class="btn btn-sm btn-primary">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <form action="{{ route('sous-categories.destroy', $sousCategorie) }}"
                                          method="POST"
                                          class="d-inline"
                                          onsubmit="return confirm('Voulez-vous vraiment supprimer cette sous-catégorie ?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger">
                                            <i class="bi bi-trash"></i>
                                        </button>
                                    </form>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-center">Aucune sous-catégorie trouvée</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                {{ $sousCategories->links('vendor.pagination.bootstrap-5') }}
            </div>
        </div>
    </div>
</div>
@endsection
